...
public class TeleportVive : MonoBehaviour {
    ...
    public TeleportSpellController teleportSpellController;
    ...
    public float TeleportTimeMarker = -1;
	...
	void Update ()
    {
        ...
        if(CurrentTeleportState == TeleportState.Teleporting)
        {
            ...
            if(Time.time - TeleportTimeMarker >= TeleportFadeDuration / 2)
            {
                ...
                } else
                {
                    ...
                    OriginTransform.rotation = Pointer.SelectionPadObject.transform.rotation;
                }
        ...
        else if(CurrentTeleportState == TeleportState.Selecting)
        {
            ...
            //(SteamVR_Actions.default_MenuPress.lastState)
            ...
            shouldTeleport = SteamVR_Actions.default_Teleport.GetStateUp(SteamVR_Input_Sources.LeftHand);
            bool shouldCancel = SteamVR_Actions.default_GrabGrip.GetState(SteamVR_Input_Sources.LeftHand);
            if (shouldTeleport || shouldCancel)
            {
                ...
                if (shouldTeleport && Pointer.PointOnNavMesh)
                {
                    ...
                    //TeleportTimeMarker = Time.time;
                    teleportSpellController = Instantiate(TeleportSpell).GetComponent<TeleportSpellController>();
                    teleportSpellController.Teleport(this.gameObject.transform.position,Pointer.SelectedPoint);
                    teleportSpellController.teleporter = this;
                    //teleportSpellController.device = ActiveController;
                    CurrentTeleportState = TeleportState.None;
