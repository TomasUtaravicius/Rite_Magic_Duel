...
public class DeveloperDefined : BasedGestureHandle {
    ...
    void HandleOnDeveloperDefinedMatch(long gestureId, string gesture, float score) {
        ...
        if (gesture.Equals("Protego") && score >= 1.0f)
        {
            Debug.Log("Leidziu PROTEGO");
            sManager.castProtego = true;
        }
        if (gesture.Equals("Expelliarmus") && score >= 1.0f)
        {
            Debug.Log("Leidziu Expelliarmus");
            sManager.castExpelliarmus = true;
        }
        if (gesture.Equals("Lethal") && score >= 1.0f)
        {
            Debug.Log("Leidziu Lethal");
            sManager.castLethal = true;
        }
        if (gesture.Equals("Stupefy") && score >= 1.0f)
        {
            Debug.Log("Leidziu Stupefy");
            sManager.castStupefy = true;
        }
        if (gesture.Equals("Protego_Max") && score >= 1.0f)
        {
            Debug.Log("Leidziu Protego max");
            //sManager.castStupefy = true;
        }
    }
    ...
    void Awake() {
       ...
       // textResult.text = defaultResultText = "Pressing trigger and write symbol in the air\nReleasing trigger when finish";
        ...
        airsigManager.SetDeveloperDefinedTarget(new List<string> { "Expelliarmus", "Protego", "Protego_Max", "Stupefy", "Lethal" });
        airsigManager.SetClassifier("FirstSpellSet", "");
